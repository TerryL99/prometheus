...
/*
Can be used for Prometheus functions: quantile_over_time
*/
...
func (s *KLLSampler) ReceiveItem(item float64, item_weight int64) (float64, bool) {
	// default value of item_weigth = 1
	if s.h == 0 {
		return item, true
	} else {
...
func (c *Compactor) compact(deletion bool) []float64 {
	...
	if deletion {
		...
		// fmt.Println("number_of_remove =", number_of_remove)
	...
	// Push(E_i, e_i+1)
	sort.Slice(c.list, func(i, j int) bool { return c.list[i] < c.list[j] })
	if c.alternate  && c.numCompaction%2 == 1 {
		c.offset = 1 - c.offset
	} else {
		if rand.Float32() < 0.5 {
			c.offset = 1
	...
	/*
		c.list = c.list[:0] // remain the allocated memory
